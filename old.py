...
for idx, lap in laps.pick_drivers("PIA").iterrows():
    ...
    for x_idx, x in enumerate(allX):
        lapDataPoints[x_idx]['x'] = x

    for y_idx, y in enumerate(allY):
        lapDataPoints[y_idx]['y'] = y
    ...
    for dataPoint in lapDataPoints:
        ...
        if currentCornerRange[0] <= distance < currentCornerRange[1]:
            dataPoint['deltaDistance'] = distance - currentCornerRange[0]
...
for corner in cornerZones:
    ...
    xAxis = [dataPoint.get("deltaDistance") for dataPoint in data]
